...
export default function VendorsPage() {
  ...
  const banMutation = useMutation({
    ...
    onError: () => toast.error('Failed to ban vendor'),
  })
            ...
            {pendingVendors.map((vendor) => (
              <div key={vendor.id} className="bg-yellow-50 border border-yellow-200 rounded-lg p-5 flex justify-between items-center">
                ...
                <div className="flex gap-2 ml-4">
                  ...
                  <button
                    onClick={() => { if (confirm(`Reject "${vendor.store_name}"?`)) banMutation.mutate(vendor.id) }}
              ...
              <tbody>
                {approvedVendors.map((vendor) => (
                  <tr key={vendor.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-6 py-3">
                      <button
                        onClick={() => { if (confirm(`Ban "${vendor.store_name}"?`)) banMutation.mutate(vendor.id) }}
                        disabled={banMutation.isPending}
